...
from config import DISCORD_TOKEN, TELEGRAM_TOKEN, DISCORD_CHANNEL_IDS, TELEGRAM_TARGETS, EXTRA_BRIDGES, BRIDGES
from queues import RelayQueues
from discord_handlers import setup_discord_handlers
from telegram_handlers import setup_telegram_handlers
...
# --- Legacy bridge workers (unchanged) ---
...
# --- New bridge system workers ---

async def bridge_relay_discord_worker(queues, mappings):
    """Process Discord messages and relay them via the bridge system."""
    bridge_manager = mappings["bridge_manager"]
    discord_bot = mappings["discord_bot"]
    telegram_app = mappings["telegram_app"]
    
    while True:
        item = await queues.bridge_relay_discord.get()
        channel_id = item["channel_id"]
        message_id = item["message_id"]
        body = item["body"]
        
        try:
            # Relay the message using the bridge manager
            await bridge_manager.relay_from_discord(
                discord_bot, 
                telegram_app, 
                channel_id, 
                message_id, 
            ...
            print(f"[Bridge System] Error relaying Discord message: {e}")

async def bridge_relay_telegram_worker(queues, mappings):
    """Process Telegram messages and relay them via the bridge system."""
    bridge_manager = mappings["bridge_manager"]
    discord_bot = mappings["discord_bot"]
    telegram_app = mappings["telegram_app"]
    
    # Wait for Discord to be ready before processing messages
    await discord_bot.wait_until_ready()
    
    while True:
        item = await queues.bridge_relay_telegram.get()
        ...
        try:
            # Relay the message using the bridge manager
            await bridge_manager.relay_from_telegram(
                discord_bot, 
                telegram_app, 
                chat_id, 
                topic_id, 
            ...
            print(f"[Bridge System] Error relaying Telegram message: {e}")

async def bridge_edit_discord_worker(queues, mappings):
    """Process Discord message edits and apply them via the bridge system."""
    bridge_manager = mappings["bridge_manager"]
    discord_bot = mappings["discord_bot"]
    telegram_app = mappings["telegram_app"]
    
    while True:
        item = await queues.bridge_edit_discord.get()
        channel_id = item["channel_id"]
        message_id = item["message_id"]
        body = item["body"]
        
        try:
            # Edit all copies of this message using the bridge manager
            await bridge_manager.edit_discord_message(
                discord_bot, 
                telegram_app, 
                channel_id, 
                message_id, 
            ...
            print(f"[Bridge System] Error editing Discord message: {e}")

async def bridge_edit_telegram_worker(queues, mappings):
    """Process Telegram message edits and apply them via the bridge system."""
    bridge_manager = mappings["bridge_manager"]
    discord_bot = mappings["discord_bot"]
    telegram_app = mappings["telegram_app"]
    
    # Wait for Discord to be ready before processing edits
    await discord_bot.wait_until_ready()
    
    while True:
        item = await queues.bridge_edit_telegram.get()
        chat_id = item["chat_id"]
        topic_id = item["topic_id"]
        message_id = item["message_id"]
        body = item["body"]
        ...
        try:
            # Edit all copies of this message using the bridge manager
            await bridge_manager.edit_telegram_message(
                discord_bot, 
                telegram_app, 
                chat_id, 
                topic_id, 
            ...
            print(f"[Bridge System] Error editing Telegram message: {e}")

async def bridge_delete_discord_worker(queues, mappings):
    """Process Discord message deletions and apply them via the bridge system."""
    bridge_manager = mappings["bridge_manager"]
    discord_bot = mappings["discord_bot"]
    telegram_app = mappings["telegram_app"]
    
    while True:
        item = await queues.bridge_delete_discord.get()
        channel_id = item["channel_id"]
        message_id = item["message_id"]
        
        try:
            # Delete all copies of this message using the bridge manager
            await bridge_manager.delete_discord_message(
                discord_bot, 
                telegram_app, 
                channel_id, 
                message_id
            ...
            print(f"[Bridge System] Error deleting Discord message: {e}")

async def bridge_delete_telegram_worker(queues, mappings):
    """Process Telegram message deletions and apply them via the bridge system."""
    bridge_manager = mappings["bridge_manager"]
    discord_bot = mappings["discord_bot"]
    telegram_app = mappings["telegram_app"]
    
    # Wait for Discord to be ready before processing deletions
    await discord_bot.wait_until_ready()
    
    while True:
        item = await queues.bridge_delete_telegram.get()
        chat_id = item["chat_id"]
        topic_id = item["topic_id"]
        message_id = item["message_id"]
        ...
        try:
            # Delete all copies of this message using the bridge manager
            await bridge_manager.delete_telegram_message(
                discord_bot, 
                telegram_app, 
                chat_id, 
                topic_id, 
            ...
            print(f"[Bridge System] Error deleting Telegram message: {e}")
...
async def main():
    ...
    # Initialize the bridge manager with configured bridges
    bridge_manager = BridgeManager(BRIDGES)
    mappings["bridge_manager"] = bridge_manager
    ...
    # Start new bridge system workers
    asyncio.create_task(bridge_relay_discord_worker(queues, mappings))
    asyncio.create_task(bridge_relay_telegram_worker(queues, mappings))
    asyncio.create_task(bridge_edit_discord_worker(queues, mappings))
    asyncio.create_task(bridge_edit_telegram_worker(queues, mappings))
    asyncio.create_task(bridge_delete_discord_worker(queues, mappings))
    asyncio.create_task(bridge_delete_telegram_worker(queues, mappings))
