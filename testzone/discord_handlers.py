import discord
from config import DISCORD_CHANNEL_IDS, EXTRA_BRIDGES
...
def setup_discord_handlers(bot, queues, mappings):
    ...
    async def on_message(message):
        ...
        if relay_channel_id:
            await queues.discord_to_telegram.put(((relay_channel_id, message.id), body))

            # Crosspost to other Discord channels
            for dst_chan_id in DISCORD_CHANNEL_IDS:
                if dst_chan_id == relay_channel_id:
                    continue
                dst_channel = bot.get_channel(dst_chan_id)
                if dst_channel:
                    try:
                        sent = await dst_channel.send(body)
                        mappings['discord_crosspost'].setdefault((relay_channel_id, message.id), {})[dst_chan_id] = sent.id
                    except Exception as e:
                        print(f"[Discord] Crosspost error: {e}")

        # --- LEGACY: Extra bridge handling ---
        ...
        # Check if this channel is part of any configured bridge
        if mappings["bridge_manager"].find_bridges_for_discord_channel(channel_id):
            # Send via the new bridge system
            await queues.bridge_relay_discord.put({
    ...
    async def on_message_edit(before, after):
        ...
        if relay_channel_id:
            for key, tg_msg_id in list(mappings["discord_to_telegram"].items()):
                ...
                if d_chan_id == relay_channel_id and d_msg_id == after.id:
                    try:
            ...
            crossposts = mappings["discord_crosspost"].get((relay_channel_id, after.id), {})
        ...
        # --- LEGACY: Extra bridge edit handling ---
        for idx, bridge in enumerate(EXTRA_BRIDGES):
        ...
        if mappings["bridge_manager"].find_bridges_for_discord_channel(channel_id):
            # Send edit via the new bridge system
            await queues.bridge_edit_discord.put({
                "channel_id": channel_id,
                "message_id": after.id,
                "body": body,
            })
    ...
    async def on_message_delete(message):
        ...
        if relay_channel_id:
            for key in list(mappings["discord_to_telegram"].keys()):
                tg_chat_id, tg_topic_id, d_chan_id, d_msg_id = key
                if d_chan_id == relay_channel_id and d_msg_id == message.id:
            ...
            crossposts = mappings["discord_crosspost"].pop((relay_channel_id, message.id), {})
            for dst_chan_id, dst_msg_id in crossposts.items():
                dst_channel = bot.get_channel(dst_chan_id)
                if dst_channel:
                    try:
        ...
        # --- LEGACY: Extra bridge delete handling ---
        for idx, bridge in enumerate(EXTRA_BRIDGES):
            if message.channel.id == bridge["discord_channel_id"]:
                await queues.bridge_discord_edit_delete.put({
                    "action": "delete",
        ...
        # --- NEW: Bridge system delete handling ---
        if mappings["bridge_manager"].find_bridges_for_discord_channel(channel_id):
            # Send delete via the new bridge system
            await queues.bridge_delete_discord.put({
                "channel_id": channel_id,
                "message_id": message.id,
            })
