...
def find_relay_group_for_discord(channel_id):
    """Находит группу ретрансляции для данного ID канала Discord."""
    for i, group in enumerate(RELAY_GROUPS):
        if channel_id in group["discord_channels"]:
            return i, group
    return None, None
